...
def main(hydra_config, global_hydra_config):
    ...
    if wandb_run is not None :
        ...
        # wandb_run.save(log_config_path) ### Mathias : to make offline sync work
...
def run(rank, gpu_list, dataset, wandb_run, hydra_config, global_hydra_config):
    ...
    # Select device and set it as current CUDA device for this rank
    if len(gpu_list) == 0:
        device = 'cpu'
    else:
        local_gpu = gpu_list[rank] if len(gpu_list) > 1 else gpu_list[0]
