...
srun apptainer exec --nv \
  ...
  --pwd  "$WORKDIR" \
  "$IMAGE" \
  bash -lc 'SPCONV_ALGO=native CUDA_LAUNCH_BLOCKING=1 HYDRA_FULL_ERROR=1 \
  python -m main_multiring --config-name segmentation_test \
  hydra.run.dir=/workspace/work/ml/outputs/2026-01-06/04-44-30/test/${SLURM_JOB_ID} \
